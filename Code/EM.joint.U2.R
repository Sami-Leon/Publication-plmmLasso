...
Estep <- function(Data, out.F, g, ni, theta, fixed_effects) {
...
MstepSe <- function(Data, out.F, g, se, U2, ni, theta, fixed_effects) {
  X <- as.matrix(cbind(1, subset(Data, select = -c(series, position, Y))))
...
EM.joint <- function(Y, series, position, X = NULL, F.Bases, gam.cste = 0, intercept,
  ...
  while ((delta.EM > tol.EM) & (Iter < maxIter)) {
    ...
    out.E <- Estep(Data, out.F, g, ni, theta, fixed_effects)
    U2 <- out.E$U2

    su.tmp <- MstepSu(Data, g, se, U2, ni)
